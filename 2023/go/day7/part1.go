...
var card_strength = []string{"2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A"}
...
type rankedHand struct {
	h    hand
	rank int // this is based on what type of hand they have
}

type rankedHandList []rankedHand

func (rhl rankedHandList) Len() int {
	return len(rhl)
}

func (rhl rankedHandList) Less(i, j int) bool {
	if rhl[i].rank == rhl[j].rank {
		for index, card_i := range rhl[i].h.cards {
			card_j := rhl[j].h.cards[index]
			if card_i != card_j {
				return slices.Index(card_strength, card_i) > slices.Index(card_strength, card_j)
			}
		}
	}
...
func (rhl rankedHandList) Swap(i, j int) {
	rhl[i], rhl[j] = rhl[j], rhl[i]
}

func rankHand(h hand) rankedHand {
...
func Part1Solution(input []string) string {
	...
	ranked_hands := rankedHandList{}
	for _, h := range hands {
		ranked_hands = append(ranked_hands, rankHand(h))
	}
